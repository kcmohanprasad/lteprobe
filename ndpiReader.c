 ...
 * @details TODO
 */
void setupDetection(u_int16_t thread_id);
...
char* ipProto2Name(uint8_t proto_id);
char* intoaV4(unsigned int addr, char* buf, u_short bufLen);
void printFlow(u_int16_t thread_id, struct ndpi_flow *flow);
void free_ndpi_flow(struct ndpi_flow *flow);
void ndpi_flow_freer(void *node);
...
void node_count_walker(void *node, ndpi_VISIT which, int depth, void *user_data); 
void node_print_unknown_proto_walker( const void *node, ndpi_VISIT which, int depth, void *user_data);
...
void node_print_known_proto_walker(const void *node, ndpi_VISIT which, int depth, void *user_data);
void dpiModule ( struct rte_mbuf *);
void *
ndpi_tfindLocal(const void *vkey, void *vrootp);
int ndpi_tnodeCmpLocal ( const void *a , const void *b);
...
//***********************************************************
void
dpiModule ( struct rte_mbuf *m )
        {
  uint16_t lcore_id = 0x0;
  ...
  const struct ndpi_ip6_hdr *iph6;
  u_int64_t time;
  u_int16_t type=0x0, ip_offset=0x0, ip_len;
...
// printf("[ndpiReader] pcap_packet_callback : [%u.%u.%u.%u.%u -> %u.%u.%u.%u.%u]\n", ethernet->h_dest[1],ethernet->h_dest[2],ethernet->h_dest[3],ethernet->h_dest[4],ethernet->h_dest[5],ethernet->h_source[1],ethernet->h_source[2],ethernet->h_source[3],ethernet->h_source[4],ethernet->h_source[5]);
  ...
  time = ((uint64_t) begin.tv_sec) * detection_tick_resolution +
    begin.tv_usec / (1000000 / detection_tick_resolution);
  ...
  ndpi_thread_info[thread_id].last_time = time;
    ...
    ethernet = (const struct ndpi_ethhdr *) packet;
  ...
  iph = (const struct ndpi_iphdr *) &packet[ip_offset];
  //...this is where the ip hdr starts
  pktLen = ip_offset + ntohs(iph->tot_len);

  // just work on Ethernet packets that contain IP
  
  if(type == ETH_P_IP && pktLen>= ip_offset) {
  ...
  if(iph->version == 4) {
    ...
    v4_frags_warning:
     ...
     ndpi_thread_info[thread_id].stats.total_discarded_bytes +=  pktLen;
     return;
    }
  } else if(iph->version == 6) {
    iph6 = (const struct ndpi_ip6_hdr *)&packet[ip_offset];
  ...
  if(decode_tunnels && (proto == IPPROTO_UDP)) {
   const struct ndpi_udphdr *udp = (const struct ndpi_udphdr *)&packet[ip_offset+ip_len];
  ...
  // process the packet
  protocolIdentified = packet_processing(thread_id, time, iph, iph6, ip_offset, (u_int16_t)(pktLen - ip_offset), pktLen);
  return (protocolIdentified);
  }
...
static void debug_printf(u_int32_t protocol, void *id_struct,
  ...
  if(log_level <= nDPI_traceLevel) {
    ...
    snprintf(out_buf, sizeof(out_buf), "proto=%d %s %s%s", protocol, theDate, extra_msg, buf);
    printf("%s", out_buf);
    fflush(stdout);
  }
...
char* ipProto2Name(uint8_t proto_id) {
  ...
  case IPPROTO_IGMP:
    //return("IGMP");
    snprintf(proto, sizeof(proto), "%s", "IGMP");
    break;
  }
...
void node_idle_scan_walker(const void *node, ndpi_VISIT which, int depth, void *user_data) {
  const struct ndpi_flow *pTempFlow1 = node;
  const struct ndpi_flow **pTempFlow2 = &pTempFlow1;
  const struct ndpi_flow *flow = *pTempFlow2;
  //struct ndpi_flow *flow = *(const struct ndpi_flow **) node;
  uint16_t thread_id = *((uint16_t *) user_data); 
  if (depth) {};//keep compiler happy
  ...
  if((which == ndpi_preorder) || (which == ndpi_leaf)) { /* Avoid walking the same node multiple times */
    if(flow->last_seen + MAX_IDLE_TIME < ndpi_thread_info[thread_id].last_time) {
      free_ndpi_flow((struct ndpi_flow *)flow);
...
void node_count_walker(void *node, ndpi_VISIT which, int depth, void *user_data) {
  struct ndpi_flow *flow = *(struct ndpi_flow**)node;
  uint16_t num = *((uint16_t*)user_data);
  if ((flow==NULL)||(depth==0)){};//satisfy compiler
...
void node_print_unknown_proto_walker( const void *node, ndpi_VISIT which, int depth, void *user_data) {
  const struct ndpi_flow *flow = *(const struct ndpi_flow**)node;
  uint16_t thread_id = *((uint16_t*)user_data);
  ...
  if (depth==0) {};//keep compiler happy};
}

/* ***************************************************** */

void node_print_known_proto_walker(const void *node, ndpi_VISIT which, int depth, void *user_data) {
  const struct ndpi_flow *flow = *(struct ndpi_flow**)node;
...
/* ***************************************************** */
 ...
 int node_cmp( const void *a,  const void *b) {
  const struct ndpi_flow *fa = (struct ndpi_flow*)a;
  const struct ndpi_flow *fb = (struct ndpi_flow*)b;
...
ndpi_tfindLocal(const void *vkey, void *vrootp)
{
  ...
  const struct ndpi_flow *fa = (struct ndpi_flow *) key;
  const struct ndpi_flow *fb = (struct ndpi_flow *) (*rootp)->key;
  ...
  while (*rootp != (ndpi_node *)0) {    /* T1: */
    ...
    printf ("W");
    const struct ndpi_flow *fb = (struct ndpi_flow *) (*rootp)->key;
  //printf ("sks: comparing fa(lip(0x%x),lprt(0x%x),uip(0x%x),uport(0x%x)) fb(lip(0x%x),lprt(0x%x),uip(0x%x),uport(0x%x))\n",fa->lower_ip,fa->lower_port,fa->upper_ip,fa->upper_port,fb->lower_ip,fb->lower_port,fb->upper_ip,fb->upper_port);

    if ((fa->lower_ip < fb->lower_ip)||(fa->lower_port < fb->lower_port)||(fa->upper_port < fb->upper_port)||(fa->protocol < fb->protocol)) r=-1;
    else if ((fa->lower_ip > fb->lower_ip)||(fa->lower_port > fb->lower_port)||(fa->upper_port > fb->upper_port)||(fa->protocol > fb->protocol)) r=1;
    else r=0;
...
static struct ndpi_flow *get_ndpi_flow(u_int16_t thread_id,
  ...
  *proto = iph->protocol;
  ...
  //ret = ndpi_tfindLocal(&flow, &ndpi_thread_info[thread_id].ndpi_flows_root[idx]);
  ...
  if(ret == NULL) {
	...
	return(NULL);
      ...
      void * q=ndpi_tsearch(newflow, &ndpi_thread_info[thread_id].ndpi_flows_root[idx], node_cmp); /* Add */
      if (q==NULL) printf ("could not add new flow to root tree\n");
      ndpi_thread_info[thread_id].stats.ndpi_flow_count++;
...
void setupDetection(uint16_t thread_id) {
...
static unsigned int packet_processing(u_int16_t thread_id,
  ...
  if(flow != NULL) {
    ...
    printf ("fow is null, returning NULL\n");
    return(0);
  }

  if(flow->detection_completed) { /*printf ("d_proto=detection done\n")*/;return(flow->detected_protocol);};
 ...
 // printf ("d_proto=0x%x guess_proto=",flow->detected_protocol);
...
static void pcap_packet_callback(u_char *args, const struct pcap_pkthdr *header, const u_char *packet) {
  ...
  if(ndpi_thread_info[thread_id]._pcap_datalink_type == DLT_NULL) {
    if(ntohl(*((const u_int32_t*)packet)) == 2)
      type = ETH_P_IP;
    else
      type = 0x86DD; /* IPv6 */
...
/*int dpiMain(int argc, char **argv) {
  int i;
